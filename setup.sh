...
checkEnv() {
    ...
    if [[ ! -w ${DOTFILESPATH} ]]; then
        echo -e "${RED}Can't write to ${DOTFILESPATH}${RC}"
        exit 1
    fi
...
linkConfig() {
    ...
    ## Make symbolic link.
    ln -svf ${DOTFILESPATH}/.bashrc ${USER_HOME}/.bashrc
    ln -svf ${DOTFILESPATH}/starship.toml ${USER_HOME}/.config/starship.toml
}
